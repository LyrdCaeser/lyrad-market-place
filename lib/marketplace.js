...
async function scanWithLaa({filename,bytes,sha256,version='1.0.0'}) {
  ...
  if(!base || !key){const error=new Error('Máy quét LAA Sandbox chưa được cấu hình.');error.status=503;throw error;}
  ...
  }catch(cause){const error=new Error('Không thể kết nối máy quét LAA Sandbox.');error.status=503;error.cause=cause;throw error;}
  const result=await response.json().catch(()=>({}));
  if(!response.ok){const error=new Error(result.message || result.detail || 'LAA Sandbox từ chối yêu cầu quét.');error.status=response.status>=500?503:422;error.scan=result;throw error;}
  if(result.sha256!==sha256){const error=new Error('LAA trả về SHA-256 không khớp tệp gốc.');error.status=502;error.scan=result;throw error;}
  if(result.is_safe!==true){const error=new Error(result.message || 'LAA phát hiện rủi ro trong tệp.');error.status=422;error.scan=result;throw error;}
  return result;
}
...
module.exports = function marketplace(pool) {
  ...
  async function save(req,res,editing){
    ...
    try{
      ...
      const values=[id,req.actor.id,req.actor.name,b.name.trim(),b.description,b.platform,b.category,b.kind,b.price,b.payment_method,b.contact,b.file_id,status,req.actor.profileUid,version];
      await client.query(`INSERT INTO lyrad_products(id,owner_id,seller_name,name,description,platform,category,kind,price,payment_method,contact,file_id,status,seller_profile_uid,version) VALUES(${values.map((_,i)=>'$'+(i+1)).join(',')}) ON CONFLICT(id) DO UPDATE SET seller_name=EXCLUDED.seller_name,seller_profile_uid=EXCLUDED.seller_profile_uid,name=EXCLUDED.name,description=EXCLUDED.description,platform=EXCLUDED.platform,category=EXCLUDED.category,price=EXCLUDED.price,payment_method=EXCLUDED.payment_method,contact=EXCLUDED.contact,file_id=EXCLUDED.file_id,status=EXCLUDED.status,review_note='',version=EXCLUDED.version,updated_at=now()`,values);
