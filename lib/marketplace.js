...
module.exports = function marketplace(pool) {
  ...
  router.use((err,req,res,next)=>{console.error('Marketplace:',err.code || err.name);const tooLarge=err.type==='entity.too.large';res.status(tooLarge?413:err.code==='42P01'?503:400).json({error:tooLarge?'Tệp vượt giới hạn 80 MB.':err.code==='42P01'?'Chưa chạy migration 002_marketplace.sql.':err.code?'Không thể lưu dữ liệu. Vui lòng kiểm tra và thử lại.':err.message});});
  return router;
};
...
module.exports.MAX_FILE=MAX_FILE;
