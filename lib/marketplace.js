...
const MAX_USER_STORAGE = 128 * 1024 * 1024;
const MAX_ACCOUNT_STORAGE = 1024 * 1024 * 1024;
...
module.exports = function marketplace(pool) {
  ...
  async function updatePresence(actor, sessionId, online) {
    if (!actor.profileUid) return false;
    const client=await pool.connect();
    try {
      await client.query('BEGIN');
      await client.query("INSERT INTO lyrad_db_storage (db_key, db_data) VALUES ('lyrad_user_presence', '{}'::jsonb) ON CONFLICT (db_key) DO NOTHING");
      const stored=await client.query("SELECT db_data FROM lyrad_db_storage WHERE db_key='lyrad_user_presence' FOR UPDATE");
      const presence=readPresence(stored.rows[0]?.db_data);
      const now=Date.now();
      for(const value of Object.values(presence)){
        const oldSessions=readPresence(readPresence(value).sessions);
        for(const [id,lastSeen] of Object.entries(oldSessions))if(Number(lastSeen)<now-86400000)delete oldSessions[id];
      }
      const entry=readPresence(presence[actor.profileUid]);
      const sessions=readPresence(entry.sessions);
      if (online) sessions[sessionId]=now;
      else delete sessions[sessionId];
      presence[actor.profileUid]={sessions,lastSeenAt:now};
      await client.query(
        `INSERT INTO lyrad_db_storage (db_key, db_data)
         VALUES ('lyrad_user_presence', $1)
         ON CONFLICT (db_key) DO UPDATE SET db_data=$1, last_updated=CURRENT_TIMESTAMP`,
  ...
  function presenceSession(req, res) {
    ...
    if (!/^[A-Za-z0-9_-]{8,80}$/.test(sessionId)) {
      res.status(400).json({error:'Mã phiên hoạt động không hợp lệ.'});
      return null;
    }
    return sessionId;
  }
  router.post('/presence',signed,wrap(async(req,res)=>{
    const sessionId=presenceSession(req,res);if(!sessionId)return;
    if(!await updatePresence(req.actor,sessionId,true))return res.status(409).json({error:'Tài khoản chưa có Hồ Sơ Creator.'});
    res.json({online:true});
  }));
  router.delete('/presence',signed,wrap(async(req,res)=>{
    const sessionId=presenceSession(req,res);if(!sessionId)return;
    if(!await updatePresence(req.actor,sessionId,false))return res.status(409).json({error:'Tài khoản chưa có Hồ Sơ Creator.'});
    res.json({online:false});
  }));
  router.get('/presence',signed,privileged,wrap(async(req,res)=>{
    const stored=await pool.query("SELECT db_data FROM lyrad_db_storage WHERE db_key='lyrad_user_presence'");
    const presence=readPresence(stored.rows[0]?.db_data), cutoff=Date.now()-40000, result={};
    for(const [uid,entryValue] of Object.entries(presence)){
      const entry=readPresence(entryValue), sessions=readPresence(entry.sessions);
      result[uid]={online:Object.values(sessions).some(value=>Number(value)>=cutoff),lastSeenAt:Number(entry.lastSeenAt)||0};
    }
    res.json(result);
  }));
  ...
  router.post('/files',express.raw({type:'application/octet-stream',limit:MAX_FILE}),signed,wrap(async(req,res)=>{
    ...
    const storageLimit=['ADMIN','NPH'].includes(req.actor.role)?MAX_ACCOUNT_STORAGE:MAX_USER_STORAGE;
    if(Number(usage.rows[0].n)+bytes.length>storageLimit) return res.status(413).json({error:`Tài khoản đã đạt giới hạn lưu trữ ${storageLimit===MAX_ACCOUNT_STORAGE?'1 GB':'128 MB'}.`});
  ...
  router.post('/products/:id/version',express.raw({type:'application/octet-stream',limit:MAX_FILE}),signed,privileged,wrap(async(req,res)=>{
    ...
    try{
      ...
      if(Number(usage.rows[0].n)+bytes.length>MAX_ACCOUNT_STORAGE){await client.query('ROLLBACK');return res.status(413).json({error:'Tài khoản đã đạt giới hạn lưu trữ 1 GB.'});}
      ...
      await client.query('COMMIT');
      res.json({id:product.id,version,filename,sha256,file_size:bytes.length,verified:true});
    }catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}
  }));
...
module.exports.scanWithLaa=scanWithLaa;module.exports.MAX_FILE=MAX_FILE;module.exports.MAX_USER_STORAGE=MAX_USER_STORAGE;module.exports.MAX_ACCOUNT_STORAGE=MAX_ACCOUNT_STORAGE;
