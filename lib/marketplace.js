...
function decodeInstaller(filename,platform,base64) {
  ...
  if(!installerExt || typeof filename!=='string' || filename.length>200 || ![installerExt,'.zip','.rar','.7z'].includes(ext) || typeof base64!=='string' || base64.length>Math.ceil(MAX_FILE/3)*4 || !/^[A-Za-z0-9+/]*={0,2}$/.test(base64)) throw new Error('Chọn APK (Mobile), EXE (PC) hoặc ZIP, RAR, 7Z, tối đa 32 MB.');
...
module.exports = function marketplace(pool) {
  ...
  router.post('/files',signed,wrap(async(req,res)=>{
    ...
    const {bytes,sha256}=decodeInstaller(filename,platform,base64);
  ...
  router.post('/products/:id/version',signed,privileged,wrap(async(req,res)=>{
    ...
    try{
      ...
      const {bytes,sha256}=decodeInstaller(filename,product.platform,base64);
      ...
      res.json({id:product.id,version,filename,sha256,file_size:bytes.length});
    }catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}
  }));
...
module.exports.normalizeVersion=normalizeVersion;module.exports.decodeInstaller=decodeInstaller;
