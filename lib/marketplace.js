...
module.exports = function marketplace(pool) {
  ...
  router.post('/files',signed,wrap(async(req,res)=>{
    ...
    const signatures={'.apk':['504b0304'],'.exe':['4d5a'],'.zip':['504b0304','504b0506','504b0708'],'.rar':['526172211a0700','526172211a070100'],'.7z':['377abcaf271c']};
    if(!installerExt || typeof filename!=='string' || filename.length>200 || ![installerExt,'.zip','.rar','.7z'].includes(ext) || typeof base64!=='string' || base64.length>Math.ceil(MAX_FILE/3)*4 || !/^[A-Za-z0-9+/]*={0,2}$/.test(base64)) return res.status(400).json({error:'Chọn APK (Mobile), EXE (PC) hoặc ZIP, RAR, 7Z, tối đa 32 MB.'});
    const bytes=Buffer.from(base64,'base64');
    if(!bytes.length || bytes.length>MAX_FILE || !signatures[ext].some(signature=>bytes.subarray(0,signature.length/2).toString('hex')===signature)) return res.status(400).json({error:'Nội dung file không đúng định dạng.'});
  ...
  async function save(req,res,editing){
    ...
    try{
      ...
      const values=[id,req.actor.id,req.actor.name,b.name.trim(),b.description,b.platform,b.category,b.kind,b.price,b.payment_method,b.contact,b.file_id,status,req.actor.profileUid];
      await client.query(`INSERT INTO lyrad_products(id,owner_id,seller_name,name,description,platform,category,kind,price,payment_method,contact,file_id,status,seller_profile_uid) VALUES(${values.map((_,i)=>'$'+(i+1)).join(',')}) ON CONFLICT(id) DO UPDATE SET seller_name=EXCLUDED.seller_name,seller_profile_uid=EXCLUDED.seller_profile_uid,name=EXCLUDED.name,description=EXCLUDED.description,platform=EXCLUDED.platform,category=EXCLUDED.category,price=EXCLUDED.price,payment_method=EXCLUDED.payment_method,contact=EXCLUDED.contact,file_id=EXCLUDED.file_id,status=EXCLUDED.status,review_note='',updated_at=now()`,values);
